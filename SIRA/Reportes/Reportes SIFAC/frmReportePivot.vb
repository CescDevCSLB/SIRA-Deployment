...
Public Class frmReportePivot
    ...
    Private Sub CargarPivot(bool As Boolean)
        Try
            If Not bool Then
                Filtro = "(CONVERT(VARCHAR(10),FechaPago,112) >='" + Format(dtpFechaDesde.Value, "yyyyMMdd") + "' AND CONVERT(VARCHAR(10), FechaPago,112) <='" + Format(Me.dtpFechaHasta.Value, "yyyyMMdd") + "')"
            Else
                Filtro = "( CONVERT(VARCHAR(10), FechaPago, 112) = CONVERT(VARCHAR(10), GETDATE(), 112) )"
            End If
