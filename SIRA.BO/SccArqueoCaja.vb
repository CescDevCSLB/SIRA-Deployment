		...
		Set(ByVal Value As String)		
			If Not Value Is Nothing Then
				If Value.Length > 2147483645 Then
					Throw New ArgumentOutOfRangeException("Observaciones", Value.ToString(), "Valor inv?lido para SccArqueoCaja.Observaciones. La longitud del valor (" & Value.Length & ") excede la longitud m?xima de la propiedad (2147483645).")
				End If
			End If
...
#Region " Batch Update " 
	...
	public Shared Sub BatchUpdate( DS as Dataset, optional Byval pTransac as TransactionManager = Nothing )
		...
		Try
			...
			cmdInsert.CommandText = "INSERT INTO SccArqueoCaja ( Fecha, objMonedaID, EntradaEfectivo, Faltante, AprobadoPor, objCajaID, objCajeroID, objTipoPagoID, objConceptoID, Cerrado, Observaciones, FechaCreacion, UsuarioCreacion, FechaModificacion, UsuarioModificacion) VALUES ( @Fecha, @objMonedaID, @EntradaEfectivo, @Faltante, @AprobadoPor, @objCajaID, @objCajeroID, @objTipoPagoID, @objConceptoID, @Cerrado, @Observaciones, @FechaCreacion, @UsuarioCreacion, @FechaModificacion, @UsuarioModificacion)"
			...
			cmdUpdate.CommandText = "UPDATE SccArqueoCaja SET Fecha=@Fecha, objMonedaID=@objMonedaID, EntradaEfectivo=@EntradaEfectivo, Faltante=@Faltante, AprobadoPor=@AprobadoPor, objCajaID=@objCajaID, objCajeroID=@objCajeroID, objTipoPagoID=@objTipoPagoID, objConceptoID=@objConceptoID, Cerrado=@Cerrado, Observaciones=@Observaciones, FechaCreacion=@FechaCreacion, UsuarioCreacion=@UsuarioCreacion, FechaModificacion=@FechaModificacion, UsuarioModificacion=@UsuarioModificacion WHERE ArqueoID= @wArqueoID"
...
#Region " Insert "
	...
	Public Sub Insert(Optional ByRef pTransac As TransactionManager = Nothing)
        ...
        End If
		arParams(3) = New SqlParameter("@EntradaEfectivo", SqlDbType.Decimal)		
		If IsDBNull(m_EntradaEfectivo) Then
            arParams(3).Value = DBNull.Value
        Else
            arParams(3).Value = m_EntradaEfectivo
        End If
        ...
        End If
		arParams(8) = New SqlParameter("@objTipoPagoID", SqlDbType.Int)		
		If IsDBNull(m_objTipoPagoID) Then
            arParams(8).Value = DBNull.Value
        Else
            arParams(8).Value = m_objTipoPagoID
        End If
		arParams(9) = New SqlParameter("@objConceptoID", SqlDbType.Int)		
		If IsDBNull(m_objConceptoID) Then
            arParams(9).Value = DBNull.Value
        Else
            arParams(9).Value = m_objConceptoID
        End If
		arParams(10) = New SqlParameter("@Cerrado", SqlDbType.Bit)		
		If IsDBNull(m_Cerrado) Then
            arParams(10).Value = DBNull.Value
        Else
            arParams(10).Value = m_Cerrado
        End If
		arParams(11) = New SqlParameter("@Observaciones", SqlDbType.VarChar)		
		If IsDBNull(m_Observaciones) Then
            arParams(11).Value = DBNull.Value
        Else
            arParams(11).Value = m_Observaciones
        End If
